...
Version: 1.0 Date: April 2026 Prepared for: Journey Beyond Asia (JBA) Operations Team Event: WSB 2027 China Super Trip — Beijing & Shanghai
...
What They See
How They Access It
Travelers (guests, invitees, families)
Mobile app — QR code, itinerary, toolkit
Open the link on their phone
JBA Staff & Admins
Operations panel — scanner, master table, hotels, flights, events
Open the link on phone or computer
...
Select your staff account to log in / Enter log in credentials to log in
You land directly on the QR Scanner — the most-used feature
3.2 User Roles
Role
What They Can Do
Staff
Scanner only by default. Super Admin can grant additional access (master table, hotels, flights, events) per staff member.
Admin
View all data (master table, hotels, flights, events). Cannot edit or add data.
Super Admin
Full access — add/edit/delete travelers, upload CSV, manage hotels and flights, view audit log, manage staff accounts and permissions.


Admins see a "View Only" badge in the header. Super Admins see their full toolkit. Staff see only the pages they've been authorized to access.
...
Default For
Scanner
QR code scanning and scan history
All staff
Master Table
View the traveler database
Admins only
Hotels
View hotel assignments and room details
Admins only
Flights
View flight manifests and passenger lists
Admins only
Events
View event attendance tracking
Admins only
Audit Log
...
Go to Sidebar → Staff → "Create Staff Account"
Fill in: First Name, Last Name, Email, Role (Staff / Admin)
Save → account is created with default permissions
Adjust permissions as needed using the checkboxes
...
Session — What are you scanning for?
...
Point your phone camera at the traveler's QR code
The app automatically reads the code (no button press needed)
A result card appears immediately:
...
✅ Checked In
Green
Traveler is on the list and in the correct fleet
Let them through
⚠️ Wrong Fleet
Yellow
Traveler is assigned to a different fleet
See section 4.3 below
ℹ️ Already Scanned
Blue
This traveler was already scanned for this session
They may have already checked in
❌ Not Assigned
Red
Traveler is not on the list for this event
Direct them to the help desk / Find information directly in master list on the app
❌ Not Found
Red
QR code not recognized
Ask for their name, use manual search / Find information directly in master list on the app
🚫 Token Revoked
Red
QR code has been deactivated
Direct them to the help desk for a new QR
...
Tap "🔍 Search" in the top right
Enter the traveler's first name and last name
Tap "Go"
Select the correct person from the results
The check-in processes normally
...
How
Add a traveler
Click "➕ Add Traveler" → fill in the form → Save
Upload travelers from CSV (Must use template file)
Click "📤 Upload CSV" → select a .csv file → review results
Edit a traveler field
Click on any editable cell → change the value → press Enter
Change check-in status
Click the status badge → select new status from dropdown
Export to CSV
Click "Export CSV" → downloads a file with all current filters applied
...
10.3 Hotel Check-in
Staff at the hotel front desk open Scanner → "Hotel Check-in"
Scan each traveler's QR as they arrive
Check the Hotels page to see room assignments and roommate pairings
10.4 Adding a Last-Minute Traveler
Super Admin opens Master Table → "➕ Add Traveler"
Fill in: First Name, Last Name, Email, Booking ID, Role Type
Save → traveler appears in the system with a QR code
Share the login link with the traveler
10.5 Handling a Lost/Damaged QR Code
Traveler reports to the help desk
Staff uses Manual Search (first + last name) to find the traveler
Staff can check them in manually
Super Admin can reissue/find a QR code and send it to traveler via bluetooth or airdrop etc.
...
See my schedule
Home → My Itinerary
Show QR for my family
Home → Family Wallet
Get hotel address for taxi
Home → Toolkit → Taxi Card
Learn Chinese phrases
Home → Toolkit → Phrasebook
Convert currency
Home → Toolkit → Currency Converter
Find emergency numbers
Home → Toolkit → Emergency Info

For JBA Staff
I want to...
Go to...
Scan QR codes
Scanner (first page after login)
Search for a traveler
Scanner → 🔍 Search
See all travelers
Sidebar → Master Table
See my hotel's guests
Sidebar → Hotels → click your hotel
See flight passengers
Sidebar → Flights → click a flight
See event attendance
Sidebar → Events
Add a new traveler
Master Table → ➕ Add Traveler (Super Admin)
Upload traveler CSV
Master Table → 📤 Upload CSV (Super Admin)
Export data
Master Table → Export CSV (Super Admin)
View change history
Sidebar → Audit Log (Super Admin)
Create staff/admin account
Sidebar → Staff → Create Staff Account (Super Admin)
Change staff permissions
Sidebar → Staff → check/uncheck permission boxes (Super Admin)
...
Meaning
Booking ID
Unique registration reference (e.g., WSB-2027-001)
Fleet
A bus/group assignment for events (e.g., Fleet 8)
Pax Type
Passenger type: Adult, Child, or Infant
Invitee Type
Whether the person is an Invitee (primary) or Guest (accompanying)
Check-in Status
Pending (not yet arrived), Checked In, or No Show
QR Code
The unique scannable code each traveler shows on their phone
PII
Personally Identifiable Information (email, phone, passport name)
Master Table
The main database of all travelers with all their information
Scan Mode / Session
The context for scanning (Hotel Check-in, City Tour, Gala, etc.)
Staff Permission
A specific access right (e.g., scanner, master_table, hotels) that Super Admins can grant or revoke per staff member
Staff Management
The Super Admin page for creating staff accounts and managing their permissions
Force Check-in
Override a wrong-fleet warning to check in a traveler anyway (logged for accountability)
Flight Manifest
The passenger list for a specific flight, organized by group and hotel
